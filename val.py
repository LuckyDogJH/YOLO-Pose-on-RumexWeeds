...
from utils.general import (LOGGER, Profile, check_dataset, check_img_size, check_requirements, check_yaml,
                           ...
                           scale_boxes, scale_rootPoints, xywh2xyxy, xyxy2xywh)
...
def save_one_txt(predn, save_conf, shape, file):
    ...
    for *xyxy, conf, cls, root_xy, root_conf in predn.tolist():
        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
        line = (cls, *xywh, conf, *root_xy, root_conf) if save_conf else (cls, *xywh, *root_xy)  # label format
...
def process_batch(detections, labels, iouv):
    ...
    Arguments:
        detections (array[N, 6]), num_pred_bbox, [x_min, y_min, x_max, y_max, conf, class]
        labels (array[M, 5]), num_GT_bbox, [class, x_min, y_min, x_max, y_max]
        iouv (array [1, 10]), torch.linspace(0.5, 0.95, 10)
    Returns:
        correct (array[N, 10]), for 10 IoU_thresh levels
    """
    correct = np.zeros((detections.shape[0], iouv.shape[0])).astype(bool)
    matches_AP50 = np.zeros((0, 2), dtype=np.long)
    iou = box_iou(labels[:, 1:], detections[:, :4])  # iou.size() = num_GT_bbox, num_pred_bbox
    correct_class = labels[:, 0:1] == detections[:, 5]  # correct_class.size() = num_GT_bbox, num_pred_bbox
    for i in range(len(iouv)):
        ...
        if x[0].shape[0]:
            ...
            if i == 0:
                matches_AP50 = matches[:, :2]
            correct[matches[:, 1].astype(int), i] = True
    return torch.tensor(correct, dtype=torch.bool, device=iouv.device), torch.from_numpy(matches_AP50.astype(np.long)).to(iouv.device)
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Metrics
        for si, pred in enumerate(preds): # preds.size() == img_index_in_batch, num_pred_bbox, 9([x_min, y_min, x_max, y_max, final_score, category_label, root_x, root_y, root_conf])
                                          # target.size() ==  num_objects, 9 (img_index_in_batch, category, x, y, w, h, root_conf, root_x, root_y)
            ...
            if nl:
                ...
                if plots:
                    confusion_matrix.process_batch(predn[:, :6], labelsn)
    ...
    if len(matched_root) and matched_root[0].any():
        OKS = compute_OKS(*matched_root, sigma=oks_sigma, conf=root_conf_thresh)
