...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):  ## use PIL
            ...
            if label:                   ## 是否画bbox中class的标签以及confidence
    ...
    def circle(self, center_xy, boundary, r=5, fill=(0, 204, 255)):
        if self.pil:
            x_min = int(np.clip(center_xy[0]-r, boundary[0], boundary[2]))
            y_min = int(np.clip(center_xy[1]-r, boundary[1], boundary[3]))
            x_max = int(np.clip(center_xy[0]+r, boundary[0], boundary[2]))
            y_max = int(np.clip(center_xy[1]+r, boundary[1], boundary[3]))
            xy = (x_min, y_min, x_max, y_max)
            self.draw.ellipse(xy, fill=fill)
        else:
            cv2.circle(self.im, (int(center_xy[0]), int(center_xy[1])), int(r), (255, 204, 0), -1)
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    ## draw root points
                    if root_conf[j] >= 0.5:
                        annotator.circle(root_xy[j], boundary=[x, y, x + w, y + h], r=5, fill=(0, 204, 255))
